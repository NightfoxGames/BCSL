...
namespace BCSL
{
    public static class BasicUtil
    {
        ...
        {
            ratio = BasicMath.Clamp(ratio, 0.25f, 1f);
        ...
        public static Vector2 Transform(Vector2 value, BasicTransform transform)
        {
            return new Vector2(
        ...
        public static Vector2 Transform(float x, float y, BasicTransform transform)
        {
            return new Vector2(
        ...
        public static bool IntersectBoxes(BasicBox a, BasicBox b)
        {
            if( a.Max.X <= b.Min.X || 
                b.Max.X <= a.Min.X ||
                a.Max.Y <= b.Min.Y || 
        ...
        public static bool IntersectBoxes(BasicBox a, BasicBox b, out Vector2 mtv)
        {
            mtv = Vector2.Zero;

            if(!BasicUtil.IntersectBoxes(a, b))
            {
                return false;
            }
        ...
        public static bool IntersectSegments(Vector2 a, Vector2 b, Vector2 c, Vector2 d, out Vector2 ip)
        {
            ...
            if(!BasicUtil.IntersectSegmentsFast(a.X, a.Y, b.X, b.Y, c.X, c.Y, d.X, d.Y))
            {
                return false;
            }
        ...
        public static bool IntersectSegments(Vector2 a, Vector2 b, Vector2 c, Vector2 d)
        {
            ...
            float c1 = BasicMath.Cross(ab, ac);
            float c2 = BasicMath.Cross(ab, ad);

            float c3 = BasicMath.Cross(cd, ca);
            float c4 = BasicMath.Cross(cd, cb);
        ...
        public static void CreateBoundingBox(Vector2[] vertices, int[] hull, out BasicBox box)
        {
            float xmin = float.MaxValue;
            float xmax = float.MinValue;
            float ymin = float.MaxValue;
            ...
            box = new BasicBox(xmin, xmax, ymin, ymax);
        }

        public static void CreateBoundingBox(Vector2[] vertices, out BasicBox box)
        {
            float xmin = float.MaxValue;
            float xmax = float.MinValue;
            float ymin = float.MaxValue;
        ...
        public static bool IsPointInTriangle(Vector2 p, Vector2 a, Vector2 b, Vector2 c)
        {
            ...
            float c1 = BasicMath.Cross(ab, ap);
            float c2 = BasicMath.Cross(bc, bp);
            float c3 = BasicMath.Cross(ca, cp);
        ...
        {
            return MathF.Abs(a - b) <= BasicUtil.VerySmallAmount;
        }

        public static bool IsNearlyEqual(Vector2 a, Vector2 b)
        {
            return
                (MathF.Abs(a.X - b.X) <= BasicUtil.VerySmallAmount) && 
                (MathF.Abs(a.Y - b.Y) <= BasicUtil.VerySmallAmount);
        }
    }
}
