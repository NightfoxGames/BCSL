...
namespace BCSL
{
    public readonly struct BasicRay
    {
        public readonly Vector2 Position;
        public readonly Vector2 Direction;

        public BasicRay(Vector2 position, Vector2 direction)
        {
        ...
        public bool Intersects(in BasicCircle circle, out float distance)
        {
            distance = 0f;
            ...
            if(BasicMath.Dot(this.Direction, circle.Center - this.Position) < 0)
            {
                return false;
            }
            ...
            float c = BasicMath.Distance(this.Position, circle.Center);
            float b = BasicMath.Dot(circle.Center - this.Position, this.Direction);
        ...
        public bool Intersects(BasicEllipse ellipse, out float distance)
        {
            distance = 0f;
            ...
            BasicRay ray = new BasicRay((this.Position - ellipse.Center) * scale, Vector2.Normalize(this.Direction * scale));

            // Unit circle for ray intersection testing.
            BasicCircle circle = new BasicCircle(Vector2.Zero, 1f);
